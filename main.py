...
def get_otp():
    ...
    try:
        if otype == 'time':
            ...
            return jsonify(status='ok', type='time', username=username, key=key, otp=otp, remaining_seconds=remaining)
        ...
        return jsonify(status='ok', type='counter', username=username, key=key, otp=otp, counter_used=counter, next_counter=counter + 1)
